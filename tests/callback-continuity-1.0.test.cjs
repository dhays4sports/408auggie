...
test('every applicable finish-later surface loads browser booking with no text scheduling prompt', () => {
  for (const route of ['auto-bundle','buyer','engineers','healthcare','home','teachers','tech','life']) {
    const html = fs.readFileSync(path.join(root, route, 'index.html'), 'utf8');
    assert.match(html, /callback-scheduling-continuity\.js\?v=408-CALLBACK-1\.2/, route);
  }
...
test('tech lead submission now presents the optional CoverageFit decision before launch', () => {
  const tech = fs.readFileSync(path.join(root, 'tech/index.html'), 'utf8');
  assert.match(tech, /data-coveragefit-invitation="true"/);
  assert.match(tech, /data-coveragefit-invitation-build="408-CF-RPT-1\.3"/);
  assert.match(tech, /coveragefit-invitation\.js\?v=408-CF-RPT-1\.3/);
});
